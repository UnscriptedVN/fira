...
The module contains the base grid class responsible for making two-dimensional arrays
    organized like a grid easier to work with (`uvn_fira.api.grid.CSWorldGrid`), as well as the
...
Worlds are organized as a grid-like, two-dimensional array (matrix) consisting of rows and columns.
    ...
    position in the grid is. There are several possible options:

- `"PLAYER"`, which refers to the player character
- `"WALL"`, which refers to a wall
- `"DESK"`, which refers to a desk with an unpowered computer
...
In most scenarios, the world itself does not get modified directly since the layout of items in the
    ...
    copy of world information such as the number of devices and the player's starting position.

It may be impractical in some cases to access an element in the grid directly. The
    `uvn_fira.api.grid.CSWorldGrid` class allows for the world grid to be used in a more practical
...
class CSWorld(object):
    ...
    def walls(self):
        ...
        Returns:
            walls (uvn_fira.api.grid.CSWorldGrid): Grid containing only the walls.
        """
    ...
    def devices(self):
        ...
        Returns:
            devices (uvn_fira.api.grid.CSWorldGrid): A grid containing the devices in the world.
        """
